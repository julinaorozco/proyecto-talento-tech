...
def index():
    ...
    ax.bar(df['Fuente'], df['Producción (TWh)'], color=['orange', 'blue', 'purple', 'green', 'red'])
    ...
    plt.plot(wind_data['Year'],wind_data['Wind Capacity'],label='Capacidad Eólica',color='purple', marker='o')
    plt.plot(solar_data['Year'],solar_data['Solar Capacity'],label='Capacidad Solar',color='green',marker='x')

    plt.title('Tendencia en la Capacidad Instalada (Eólica vs Solar)')
    plt.xlabel('Año')
    plt.ylabel('Capacidad Instalda (Gigawatts)')
    plt.legend()
    plt.grid(True)
    img=BytesIO()
    plt.savefig(img, format='png')
    ...
    ax.fill_between(renewable_data['Year'],renewable_data['Total Renewable Energy'], color='purple', alpha=0.5, label='Energía Renovable')
